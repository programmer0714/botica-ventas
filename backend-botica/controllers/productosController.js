...
async function editarProducto(req, res) {
  ...
  try {
    const estado = stock < stock_minimo ? 'Stock Bajo' : 'Disponible';
    await db.query(
      `UPDATE productos SET nombre=?, id_laboratorio=?, id_categoria=?, id_presentacion=?,
       precio=?, stock=?, stock_minimo=?, fecha_vencimiento=?, estado=?
       WHERE id_producto = ?`,
      [nombre, id_laboratorio, id_categoria, id_presentacion, precio, stock, stock_minimo, fecha_vencimiento, estado, id]
    );
    res.json({ ok: true, mensaje: 'Producto actualizado correctamente' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ ok: false, mensaje: 'Error al editar producto' });
  }
}
...
module.exports = { getProductos, buscarProductos, getStockBajo, actualizarStock, crearProducto, editarProducto, eliminarProducto };